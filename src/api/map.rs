...
pub fn generate_url(is_dark: bool) -> String {
    let color_scheme = if is_dark { "dark" } else { "light" };
    let url_base = format!(
        "https://snapshot.apple-mapkit.com/api/v1/snapshot?z=12&scale=2&size=600x200&colorScheme={color_scheme}"
    );

    let mut url = Url::parse(&url_base).unwrap();

    let apple_team_id =
        std::env::var("APPLE_DEVELOPER_TEAM_ID").expect("an APPLE_DEVELOPER_TEAM_ID env var");
    let mapkit_key_id =
        std::env::var("APPLE_MAPKIT_KEY_ID").expect("an APPLE_MAPKIT_KEY_ID env var");
    ...
    let key = jsonwebtoken::EncodingKey::from_ec_pem(mapkit_key_private.as_bytes())
        .expect("a valid JWT key");
    let signature = sign(
        snapshot_path.as_str().as_bytes(),
        &key,
        jsonwebtoken::Algorithm::ES256,
    )
    .expect("a valid ES256 signature");
